...
gdp.data <- gled.data %>% 
  bind_rows(pwt.data) %>% 
  arrange(ccode, year) %>% 
  select("ccode", "year", "pop", "realgdp_merge") %>% 
  rename("gdp" = "realgdp_merge")


# V-Dem Data
vdem.data <- read_csv(here::here("Raw Data/Country_Year_V-Dem_Core_CSV_v9", "V-Dem-CY-Core-v9.csv")) %>% 
...
# Start with COW Trade Data

trade.cow.data <- read_csv(here("Raw Data", "Dyadic_COW_4.0.csv")) %>% 
  filter(year >= 1950 & ccode1 == 2) %>% 
  rename("imports_to_us" = "flow1",
         "exports_from_us" = "flow2") %>% 
  left_join(cpi, by = "year") %>% 
  mutate(imports_to_us = (imports_to_us)/cpi,
         exports_from_us = (exports_from_us)/cpi) %>% 
  select("ccode2","year", "imports_to_us", "exports_from_us") %>% 
  rename("ccode" = "ccode2")
...
exports.data <- read_xlsx(here::here("Raw Data", "Exports_to_Counterpart_Countries.xlsx"), skip = 5)
...
exports.data <- exports.data %>% 
  ...
  rename("exports_from_us" = "exports") %>% 
  select("ccode", "year", "exports_from_us")
...
imports.data <- imports.data %>% 
  ...
  rename("imports_to_us" = "imports") %>% 
  select("ccode", "year", "imports_to_us")

imf.combined <- exports.data %>% 
  full_join(imports.data, by = c("ccode", "year"))

# Combine COW and IMF trade data
trade.data.final <- trade.cow.data %>% 
  bind_rows(imf.combined) %>% 
  arrange(ccode, year)
...
troops.data.combined <- readstata13::read.dta13(here("Raw Data/troops 1950-2020.dta")) %>% 
  dplyr::select(ccode, year, troops) %>% 
  arrange(ccode, year) %>% 
  mutate(ccode = ifelse(ccode == 260, 255, ccode)) %>% 
  group_by(ccode, year) %>% 
  summarise(troops = max(troops, na.rm = TRUE)) %>% 
  group_by(ccode) %>% 
  mutate(troops = ifelse(is.finite(troops), troops, NA),
         log_troops = log1p(troops),
...
# Wrote a little code here to avoid automatically running the cshapes command if I don't need to.
if(exists("mindist") != "TRUE") {
...
for (i in 1:29) {
  ...
  mindist[[i]] <- cshapes::distlist(thedate, type = "mindist", tolerance = 0.2, useGW = FALSE)
...
undata.com <- undata %>% 
  ...
  select(ccode, ccode2, year, idealdistance) %>% 
...
ally.data <- read_csv(here("Raw Data/ATOP V4.01 Data (csv)", "atop4_01ddyr.csv")) %>% 
  ...
  select(stateB, year, us_ally) %>% 
  rename("ccode" = "stateB") %>% 
  group_by(ccode,  year) %>% 
  summarise(us_ally = max(us_ally, na.rm = TRUE)) # Get rid of Germany duplicate
...
protest.data.final <- vdem.data %>% 
  ...
  mutate_at(vars("pop", "gdp", "imports_to_us", "exports_from_us", "econaid", "milaid", "troops", "troops_w_sum", "troops_w_mean", "troops_change"),
  ...
  mutate_at(vars("gdp", "pop","exports_from_us", "imports_to_us"),
            list(growth = ~((((.) - dplyr::lag(., order_by = year)) / lag(., order_by = year))))) %>% 
  ...
  mutate_at(vars("v2x_polyarchy", "v2x_libdem", "v2x_polyarchy_c_mean", "v2x_polyarchy_c_mean_z", "v2x_polyarchy_c_demeaned_z", "v2x_polyarchy_c_demeaned", "v2x_libdem_c_mean", "v2x_libdem_c_mean_z", "v2x_libdem_c_demeaned_z", "v2x_libdem_c_demeaned", "v2x_polyarchy_z"),
            list(sq = ~(.)^2)) %>% 
  mutate(usmilsize = usmilsize/1000, # This is a really big number for some states. Divide by 1,000,000
