...
trade.com <- trade.cow.data %>% 
  ...
  across(starts_with("imports|exports"), ~round(.x, digits = 4))) %>% 
...
exports.data <- read_xlsx(here::here("Raw Data", "Exports_and_Imports_by_areas_and_Co (5).xlsx"), sheet = 1, skip = 5)
...
imports.data <-read_xlsx(here::here("Raw Data", "Exports_and_Imports_by_areas_and_Co (5).xlsx"), sheet = 2, skip = 5)
...
trade.data.final <- trade.com %>%
  ...
  filter(dups == 1) %>% 
  left_join(cpi, by = "year") %>%
  mutate(across(starts_with(c("imports", "exports")), .x = .x/cpi),
         exports_from_us = case_when(
           !is.na(exports_from_ccode1) ~ exports_from_ccode1,
           is.na(exports_from_ccode1) ~ exports_from_ccode1_imf
         ),
         imports_to_us = case_when(
           !is.na(imports_to_ccode1) ~ imports_to_ccode1,
           is.na(imports_to_ccode1) ~ imports_to_ccode1_imf))

# Check trade data comparison
tradecheck <- ggplot(data = trade.data.final %>% filter(ccode %in% c(20, 200, 210, 325, 390, 220, 255, 356, 710, 732, 740, 900)), aes(x = year)) +
  ...
  geom_line(aes(y = exports_from_us, color = "Selected"), size = 1.2) +
  facet_wrap(. ~ ccode) +
